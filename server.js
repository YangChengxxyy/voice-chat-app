...
io.on("connection", (socket) => {
  ...
  socket.on("toggle-mute", ({ roomId, userId, isMuted }) => {
    try {
      ...
      if (updatedUser) {
        ...
        console.log(
          `User ${updatedUser.name} ${isMuted ? "muted" : "unmuted"}`,
        );
      }
    } catch (error) {
  ...
  socket.on("webrtc-offer", ({ roomId, targetUserId, offer }) => {
    try {
      ...
      if (targetUser && targetUser.socketId) {
        const senderUser = Array.from(users.values()).find(
  ...
  socket.on("disconnect", (reason) => {
    console.log(
      `Client ${socket.id} disconnected: ${reason} at ${new Date().toISOString()}`,
    );
    ...
    try {
      ...
      if (user) {
        ...
        console.log(
          `User ${user.name} (${user.id}) disconnected from room ${roomId} at ${new Date().toISOString()}`,
        );
      }
    } catch (error) {
  ...
  socket.on("reconnect", () => {
    console.log(
      `Client ${socket.id} reconnected at ${new Date().toISOString()}`,
    );
    socket.emit("connection-confirmed", {
      socketId: socket.id,
      timestamp: new Date().toISOString(),
...
httpServer.listen(PORT, "0.0.0.0", () => {
  ...
  console.log(
    `CORS origins: ${process.env.NODE_ENV === "production" ? "production origins" : "http://localhost:3000"}`,
  );
});
